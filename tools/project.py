...
class ProjectConfig:
    def __init__(self) -> None:
        ...
        # Autogenerated units held back from the link because the linker emits
        # that region itself (e.g. the IMAGE_DEBUG_DIRECTORY under /debug).
        self.linker_provided_units: Optional[List[str]] = None
        # Linker flags for the base image only; modules link without them (PE).
        self.base_ldflags: Optional[List[str]] = None
...
def generate_build_ninja(
    ...
    class LinkStep:
        ...
        def write(self, n: ninja_syntax.Writer) -> None:
            ...
            if config.platform == "pe":
                ...
                if self.module_id == 0 and config.base_ldflags:
                    pe_ldflags.extend(config.base_ldflags)
    ...
    if build_config:
        ...
        linker_provided_units: Set[str] = set(config.linker_provided_units or [])
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            if obj_name in linker_provided_units:
                linker_provided_units.remove(obj_name)
                return
            obj = objects.get(obj_name)
            if obj is None:
                if not build_obj["autogenerated"]:
                    missing_config.append(f"  {obj_name}")
                if obj_path is not None:
                    link_step.add(Path(obj_path))
                return

            if obj.options["lib_member"] is None and obj.src_path is not None and not obj.src_path.exists():
                missing_source.append(f"  {obj.src_path}")
        ...
        if linker_provided_units:
            # Held-back units are named by address, so a splits.txt edit silently
            # turns the hold-back into a no-op and the region gets linked twice.
            sys.exit(f"linker_provided_units not in the build config: {sorted(linker_provided_units)}")

        if missing_source:
            sys.exit(
                "Source files not found:\n"
                + "\n".join(missing_source)
                + "\nEnsure the files exist or correct the 'source' option in configure.py."
            )
        ...
        if missing_config:
            sys.exit(
                ...
                + "\n".join(missing_config)
                + "\nAdd Object entries in configure.py's config.libs for these units."
            )
...
def generate_objdiff_config(
    ...
    for module in build_config["modules"]:
        for unit in module["units"]:
            # A PE .data section has VirtualSize > SizeOfRawData, and the zero-fill
            # tail is not covered by any split in these modules, so dtk's filler
            # symbol runs past the raw data and objdiff rejects the unit
            # ("size out of section bounds"). Drop just that one autogenerated
            # whole-section blob; every code unit is still reported.
            if (
                config.platform == "pe"
                and unit.get("autogenerated", False)
                and unit.get("code_size", 0) == 0
                and unit["name"].endswith("_data")
            ):
                continue
            progress_categories = []
