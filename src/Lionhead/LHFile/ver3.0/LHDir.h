...
// LHOSFile::ConvertDirInfo). day_of_week and the trailing sub-second field are
...
{
	char                 name[260];          /* 0x0 */
	char                 alternate_name[14]; /* 0x104 */
	uint32_t             attributes;         /* 0x114 */
	uint32_t             size;               /* 0x118 */
	struct LHDirDateTime creation_time;      /* 0x11c */
	struct LHDirDateTime write_time;         /* 0x126 */
	HANDLE               find_handle;        /* 0x130 */
	uint32_t             search_attributes;  /* 0x134 */
	WIN32_FIND_DATAA     find_data;          /* 0x138 */
};
