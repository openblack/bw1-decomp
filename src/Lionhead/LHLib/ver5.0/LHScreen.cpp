...
int LHScreen::ChangeMode(uint16_t width, uint16_t height, uint8_t depth)
{
    ...
    if (windowed)
    {
        ...
        if (pDirectDraw->CreateSurface(&desc, &pBackSurface, NULL))
        {
            ...
            MessageBoxA(
                0,
                "In Screen Settings, please increase the number of colours to more than 256",
                "Screen Depth Problem",
                MB_ICONHAND);
            return 3;
        }
...
int LHScreen::SaveBitmap()
{
    ...
    short bmpAlignPad = 0;
    (void)bmpWidth; (void)bmpHeight; (void)bmpBitCount; (void)bmpPitch;
    (void)bmpRedMask; (void)bmpGreenMask; (void)bmpBlueMask; (void)bmpAlignPad;
