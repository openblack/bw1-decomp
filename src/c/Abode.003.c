...
bool __fastcall GetNewEp__5AbodeF8ABODE_EPP7LHPoint(struct Abode* this, const void* edx, enum ABODE_EPP index, struct LHPoint* point)
{
    ...
    asm("{disp8} jl         LAB__addr_0x004035b4");                          // 0x004035b0    7c02
    asm("LAB__addr_0x004035b2:");
    asm("xor.s              eax, eax");                                      // 0x004035b2    33c0
    asm("LAB__addr_0x004035b4:");
    asm("{disp8} mov        esi, dword ptr [ecx + eax * 0x4 + 0x04]");       // 0x004035b4    8b748104
    asm("cmp.s              esi, edi");                                      // 0x004035b8    3bf7
    asm("{disp32} je        LAB__addr_0x0040367f");                          // 0x004035ba    0f84bf000000
    ...
    asm("{disp32} je        LAB__addr_0x0040367f");                          // 0x004035c8    0f84b1000000
    asm("test               ah, -0x80"); /* LH3D_MESH_FLAGS_CONTAINS_LANDSCAPE_FEATURE */  // 0x004035ce    f6c480
    asm("{disp8} je         LAB__addr_0x004035e3");                          // 0x004035d1    7410
    ...
    asm("{disp8} jmp        LAB__addr_0x00403603");                          // 0x004035fd    eb04
    asm("LAB__addr_0x004035ff:");
    asm("{disp8} mov        dword ptr [esp + 0x0c], edi");                   // 0x004035ff    897c240c
    asm("LAB__addr_0x00403603:");
    ...
    asm("{disp8} je         LAB__addr_0x00403618");                          // 0x0040360b    740b
    ...
    asm("{disp8} jmp        LAB__addr_0x0040361a");                          // 0x00403616    eb02
    asm("LAB__addr_0x00403618:");
    asm("xor.s              ebp, ebp");                                      // 0x00403618    33ed
    asm("LAB__addr_0x0040361a:");
    asm("test               dword ptr [esi + 0x04], 0x00100000"); /* LH3D_MESH_FLAGS_CONTAINS_EXTRA_METRICS */  // 0x0040361a    f7460400001000
    asm("{disp8} je         LAB__addr_0x0040362e");                          // 0x00403621    740b
    ...
    asm("{disp8} jmp        LAB__addr_0x00403630");                          // 0x0040362c    eb02
    asm("LAB__addr_0x0040362e:");
    asm("xor.s              ebx, ebx");                                      // 0x0040362e    33db
    asm("LAB__addr_0x00403630:");
    asm("test               dword ptr [esi + 0x04], 0x00200000"); /* LH3D_MESH_FLAGS_CONTAINS_EBONE */  // 0x00403630    f7460400002000
    asm("{disp8} je         LAB__addr_0x00403642");                          // 0x00403637    7409
