...
- `orig/` - User-supplied original files, none committed (ignored by `.gitignore`). Holds each version's game binary in `orig/[VERSION]/`, the MSVC 6.0 static CRT libs in `orig/libs/msvc6.<sp>/` (SP5, SP4 and RTM copies, depending on the version and the library), the Intel libraries in `orig/libs/amaths-2.0/` and `orig/libs/icc-5.0.115/`, and the DirectX 7.0 DDK in `orig/directx7.0/` (`include/` and `lib/`). See [Getting Started](docs/getting_started.md).
